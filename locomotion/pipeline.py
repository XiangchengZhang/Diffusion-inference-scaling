...
from search.tfg import TFGGuidance
...
class Pipeline:
    def __init__(self, args: Arguments = None, model=None):
        if args is None:
            args = Arguments()
        self.args = args
        ...
        if not model:
            ...
            self.model = model
        # self.sampler = DDIMSampler(args=self.args, model=self.model)
        self.sampler = TFGGuidance(args=self.args, model=self.model)
